...
from common.utils import html_escape, url_escape, html_escape_name, check_script
    ...
    def process_view(self, request, view, args, kwargs):
        try:
            ...
            if getattr(view, 'escape_script', False):
                ...
                escapeType = "url"
            # get参数转换
            request.GET = self.__escape_data(
                request.path, request.GET, escapeType)
            # post参数转换
            request.POST = self.__escape_data(
                request.path, request.POST, escapeType)
        except Exception, e:
            ...
            for _get_value in _get_value_list:
                ...
                if not is_json:
                    try:
                        ...
                        logger.error(
                            u"CheckXssMiddleware GET/POST参数 转换失败！%s" % e)
                        new_value = _get_value
                else:
                    try:
